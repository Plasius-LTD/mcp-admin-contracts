# ADR-0003: Exact-main OIDC package publishing
...
The former `@plasius/mcp-admin-contracts` workflow prepared a release commit and published it from
...
has an immutable, independently reviewable identity. The public package remains limited to metadata, types, and pure builders; release automation gains no runtime data access.
...
`Plasius-LTD`, repository `mcp-admin-contracts`, workflow `cd.yml`, environment
`production`, and action `npm publish`. GitHub `main` and `production`
policies are independent admission controls. The inherited product flags are `feedback.admin.enabled` and `feedback.mcp.enabled`; the release-governance control is `platform.public-artifact-integrity.enabled` and introduces no runtime exposure. Rollback disables
